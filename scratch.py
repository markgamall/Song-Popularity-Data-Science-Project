...
test_df = pd.read_csv("C:\\Users\\lenovo\\anaconda3\\SongPopularity.csv")
pd.set_option('display.max_columns', None)
test_df.head()
...
null_percentages = (null_counts / len(test_df)) * 100

features_to_remove = null_percentages[null_percentages <= 5].index

test_df = test_df.dropna(subset=features_to_remove)

cat_columns = ['Song', 'Album', 'Artist Names', 'Artist(s) Genres', 'Album Release Date', 'Spotify Link', 'Song Image',
               'Spotify URI', 'Hot100 Rank', 'Key', 'Mode', 'Time Signature', 'Hot100 Ranking Year']
num_columns = ['loudness', 'Acousticness', 'Energy', 'Instrumentalness', 'Song Length(ms)', 'Danceability',
               'Speechiness', 'Valence', 'Liveness', 'Tempo']

for column in cat_columns:
    if column in test_df.columns:
        mode_val = test_df[column].mode()[0]
        test_df[column].fillna(mode_val, inplace=True)

for column in num_columns:
    if column in test_df.columns:
        mean_val = test_df[column].mean()
...
# duplicate_links = df[df.duplicated(subset=['Spotify Link', 'Hot100 Ranking Year'], keep=False)]
# if len(duplicate_links) > 0.05 * len(df):
#   df.drop_duplicates(subset=['Spotify Link', 'Hot100 Ranking Year'], keep=False, inplace=True)

# empty_genre_rows = df[df['Artist(s) Genres'] == "[]"]
# 
# empty_genre_percentage = (len(empty_genre_rows) / len(df)) * 100
# 
# if empty_genre_percentage <= 5:
#     df = df[~(df['Artist(s) Genres'] == "[]")]
# else:
#     mode_val = df['Artist(s) Genres'].mode()[0]
#     df.loc[df['Artist(s) Genres'] == "[]", 'Artist(s) Genres'] = mode_val
